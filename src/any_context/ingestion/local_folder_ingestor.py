...
def run_index_folder(
    ...
    class _NativePassThroughReader:
        def load_data(self, file_path, extra_info=None):
            from llama_index.core import Document
            return [Document(text="", metadata={"file_path": str(file_path)})]

    native_extractors = {
        ".xlsx": _NativePassThroughReader(),
        ".xls": _NativePassThroughReader(),
        ".ods": _NativePassThroughReader(),
        ".pdf": _NativePassThroughReader(),
        ".png": _NativePassThroughReader(),
        ".jpg": _NativePassThroughReader(),
        ".jpeg": _NativePassThroughReader(),
        ".webp": _NativePassThroughReader(),
    }

    try:
        reader = SimpleDirectoryReader(input_files=files_to_index, file_extractor=native_extractors)
        ...
        for f_idx, single_file in enumerate(files_to_index):
            ...
            try:
                single_reader = SimpleDirectoryReader(input_files=[single_file], file_extractor=native_extractors)
